...
interface ModalReenvioProps {
  grupo: Grupo
  onClose: () => void
  onConfirmar: (ocsIds: string[], emailCC: string) => void
}
...
export default function ModalReenvio({ grupo, onClose, onConfirmar }: ModalReenvioProps) {
  ...
  function handleConfirmar() {
    ...
    onConfirmar(Array.from(selecionadas), emailCC)
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden h-80vh ">
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-gray-900">Reenviar OCs</h2>
              <p className="text-sm text-gray-400">{grupo.nome}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div className="bg-[#F5F2EC] rounded-xl px-4 py-3 text-sm text-gray-700">
            <span className="font-medium text-gray-800">E-mail principal: </span>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-gray-800">OCs a reenviar</span>
              <button
                onClick={toggleTodas}
        ...
        <div className="flex items-center justify-end gap-3 px-6 pb-6">
          <button
            onClick={onClose}
            className="px-5 py-2.5 text-sm font-medium text-gray-600 bg-[#F0EDE8] hover:bg-[#E8E4DC] rounded-full transition-colors"
          >
            Cancelar
          </button>
          <button
            onClick={handleConfirmar}
            className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-[#3A7A3A] hover:bg-[#2E6B2E] rounded-full transition-colors"
          >
            <Send size={14} />
            Confirmar reenvio
          </button>
